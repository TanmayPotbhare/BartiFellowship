...
adminlevels_Two_blueprint = Blueprint('adminlevels_Two', __name__)


def adminlevels_Two_auth(app):
    """
    ...
    @adminlevels_Two_blueprint.route('/export_level_two_applications', methods=['GET', 'POST'])
    def export_level_two_applications():
        ...
        try:
            ...
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND scrutiny_status = 'accepted'",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No records found for the year {year} in Eligibility Check Report.", 'info')
                return redirect(url_for('adminleveltwo.level_two_admin')) #return the page.
            ...
            response.headers['Content-Disposition'] = f'attachment; filename=Eligibility Check (level 2) {year}.xlsx'
            ...
            return redirect(url_for('adminleveltwo.level_two_admin'))  # Redirect to the page where year is selected.
    ...
    @adminlevels_Two_blueprint.route('/export_accepted_students_level2', methods=['GET', 'POST'])
    def export_accepted_students_level2():
        ...
        response.headers['Content-Disposition'] = 'attachment; filename= Accepted Students (Level 2).xlsx'
        response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'

        return response

    @adminlevels_Two_blueprint.route('/export_pending_students_level2', methods=['GET', 'POST'])
    def export_pending_students_level2():
        ...
        cursor.execute(
            ...
            "type_of_disability, father_name, mother_name, work_in_government, gov_department,"
            "gov_position, bank_name, account_number, ifsc_code, account_holder_name, micr FROM application_page"
            " where phd_registration_year>='2023' and scrutiny_status='pending' and status='accepted' ")
        data = cursor.fetchall()
        # Create a workbook and add a worksheet
        wb = Workbook()
        ws = wb.active
        ...
        # Create a response object and attach the workbook as a file
        response = make_response(data.getvalue())
        response.headers['Content-Disposition'] = 'attachment; filename= Pending Students (Level 2).xlsx'
        response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'

        return response

    @adminlevels_Two_blueprint.route('/export_rejected_students_level2', methods=['GET', 'POST'])
    def export_rejected_students_level2():
        ...
        # Create a response object and attach the workbook as a file
        response = make_response(data.getvalue())
        response.headers['Content-Disposition'] = 'attachment; filename= Rejected Students (Level 2).xlsx'
        response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'

        return response

    @adminlevels_Two_blueprint.route('/export_pvtg_students_level2', methods=['GET', 'POST'])
    def export_pvtg_students_level2():
        ...
        cursor.execute(
            ...
            "type_of_disability, father_name, mother_name, work_in_government, gov_department,"
            "gov_position, bank_name, account_number, ifsc_code, account_holder_name, micr FROM application_page"
            " where phd_registration_year>='2023' and your_caste IN ('katkari', 'kolam', 'madia') and status='accepted' ")
        data = cursor.fetchall()
        # Create a workbook and add a worksheet
        wb = Workbook()
        ws = wb.active
        ...
        # Create a response object and attach the workbook as a file
        response = make_response(data.getvalue())
        response.headers['Content-Disposition'] = 'attachment; filename= PVTG Students (Level 2).xlsx'
        response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'

        return response

    @adminlevels_Two_blueprint.route('/export_disabled_students_level2', methods=['GET', 'POST'])
    def export_disabled_students_level2():
        ...
        cursor.execute(
            ...
            "type_of_disability, father_name, mother_name, work_in_government, gov_department,"
            "gov_position, bank_name, account_number, ifsc_code, account_holder_name, micr FROM application_page"
            " where phd_registration_year>='2023' and disability='Yes' and status='accepted' ")
        data = cursor.fetchall()
        # Create a workbook and add a worksheet
        wb = Workbook()
        ws = wb.active
        ...
        # Create a response object and attach the workbook as a file
        response = make_response(data.getvalue())
        response.headers['Content-Disposition'] = 'attachment; filename= Disabled Students (Level 2).xlsx'
        response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
