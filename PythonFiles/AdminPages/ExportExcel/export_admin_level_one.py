...
def adminlevels_auth(app):
    ...
    def export_level_one_applications():
        ...
        try:
            ...
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND status = 'accepted'",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No records found for the year {year} in Preliminary Review Report.", 'info')
                return redirect(url_for('adminlevelone.admin_level_1')) #return the page.
            ...
            response.headers['Content-Disposition'] = f'attachment; filename=Preliminary Review (level 1) {year}.xlsx'
            ...
            return redirect(url_for('adminlevelone.admin_level_1'))  # Redirect to the page where year is selected.
    ...
    @adminlevels_blueprint.route('/export_accpeted_candidates', methods=['GET', 'POST'])
    def export_accpeted_candidates():
        if not session.get('logged_in'):
            flash('Please enter Email ID and Password', 'error')
            return redirect(url_for('adminlogin.admin_login'))
        ...
        try:
            ...
            # To export accepted applications
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND form_filled='1' AND final_approval='accepted' ",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No Accepted Applications found for the year {year}.", 'info')
                return redirect(url_for('adminlevelone.accepted_candidates')) #return the page.
            ...
            # Create a response object and attach the workbook as a file
            response = make_response(output.read())
            response.headers['Content-Disposition'] = f'attachment; filename=Accepted Applications {year}.xlsx'
            response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            ...
            print(f"An error occurred: {e}")
            flash(f"An error occurred: {e}", 'error')
            return redirect(url_for('adminlevelone.accepted_applications'))  # Redirect to the page where year is selected.
    ...
    @adminlevels_blueprint.route('/export_rejected_candidates', methods=['GET', 'POST'])
    def export_rejected_candidates():
        ...
        try:
            ...
            # To export accepted applications
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND form_filled='1' AND final_approval='rejected' ",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No Rejected Applications found for the year {year}.", 'info')
                return redirect(url_for('adminlevelone.final_rejected_candidates')) #return the page.
            ...
            # Create a response object and attach the workbook as a file
            response = make_response(output.read())
            response.headers['Content-Disposition'] = f'attachment; filename=Rejected Applications {year}.xlsx'
            response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            ...
            print(f"An error occurred: {e}")
            flash(f"An error occurred: {e}", 'error')
            return redirect(url_for('adminlevelone.final_rejected_candidates'))  # Redirect to the page where year is selected.
    ...
    @adminlevels_blueprint.route('/export_pending_candidates', methods=['GET', 'POST'])
    def export_pending_candidates():
        ...
        try:
            ...
            # To export accepted applications
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND form_filled='1' AND final_approval='pending' ",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No Pending Applications found for the year {year}.", 'info')
                return redirect(url_for('adminlevelone.final_pending_candidates')) #return the page.
            ...
            # Create a response object and attach the workbook as a file
            response = make_response(output.read())
            response.headers['Content-Disposition'] = f'attachment; filename=Pending Applications {year}.xlsx'
            response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            ...
            print(f"An error occurred: {e}")
            flash(f"An error occurred: {e}", 'error')
            return redirect(url_for('adminlevelone.final_pending_candidates'))  # Redirect to the page where year is selected.
    ...
    @adminlevels_blueprint.route('/export_disabled_candidates', methods=['GET', 'POST'])
    def export_disabled_candidates():
        if not session.get('logged_in'):
            flash('Please enter Email ID and Password', 'error')
            return redirect(url_for('adminlogin.admin_login'))
        ...
        try:
            ...
            # To export accepted applications
            cursor.execute(
                f"SELECT {', '.join(COMMON_COLUMNS)} FROM application_page WHERE phd_registration_year = %s AND form_filled='1' AND disability='Yes' ",
                (year,)
            )
            data = cursor.fetchall()

            if not data: #Check if data is empty.
                flash(f"No Disabled Applications found for the year {year}.", 'info')
                return redirect(url_for('adminlevelone.final_disabled_candidates')) #return the page.
            ...
            # Create a response object and attach the workbook as a file
            response = make_response(output.read())
            response.headers['Content-Disposition'] = f'attachment; filename=Disabled Applications {year}.xlsx'
            response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            ...
            print(f"An error occurred: {e}")
            flash(f"An error occurred: {e}", 'error')
            return redirect(url_for('adminlevelone.final_disabled_candidates'))  # Redirect to the page where year is selected.
